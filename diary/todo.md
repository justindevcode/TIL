...
---
## 20240612
### 프로그래머스 2019 카카오 개발자 겨울 인턴십 크레인 인형뽑기 게임 1단계
...
public class Main {
    ...
    static class Solution {
        ...
        public int solution(int[][] board, int[] moves) {
            ...
            Deque<Integer> relustDeque = new LinkedList<>();
            ...
            for (int i = board.length-1; i >= 0; i--) {
                for (int j = 0; j < board[0].length; j++) {
                    if (board[i][j] != 0) {
                        dequeList.get(j).addFirst(board[i][j]);
                    }
                }
            }
            for (int i = 0; i < moves.length; i++) {
                if (dequeList.get(moves[i] - 1).peekFirst() == null) {
                    continue;
                }
                Integer getObject = dequeList.get(moves[i]-1).removeFirst();
                if (Objects.equals(relustDeque.peekFirst(), getObject)) {
                    answer++;
                    Integer i1 = relustDeque.pollFirst();

                } else {
                    relustDeque.addFirst(getObject);
                }
            }
...
---
## 20240615
### 2019 카카오 개발자 겨울 인턴쉽 튜플 2단계
셀수있는 수량의 순서있는 열거 또는 어떤 순서를 따르는 요소들의 모음을 튜플(tuple)이라고 합니다. n개의 요소를 가진 튜플을 n-튜플(n-tuple)이라고 하며, 다음과 같이 표현할 수 있습니다.  
(a1, a2, a3, ..., an)튜플은 다음과 같은 성질을 가지고 있습니다. 중복된 원소가 있을 수 있습니다. ex : (2, 3, 1, 2) 원소에 정해진 순서가 있으며, 원소의 순서가 다르면 서로 다른 튜플입니다. ex : (1, 2, 3) ≠ (1, 3, 2)  
튜플의 원소 개수는 유한합니다.  원소의 개수가 n개이고, 중복되는 원소가 없는 튜플 (a1, a2, a3, ..., an)이 주어질 때(단, a1, a2, ..., an은 자연수), 이는 다음과 같이 집합 기호 '{', '}'를 이용해 표현할 수 있습니다.  
{{a1}, {a1, a2}, {a1, a2, a3}, {a1, a2, a3, a4}, ... {a1, a2, a3, a4, ..., an}}  예를 들어 튜플이 (2, 1, 3, 4)인 경우 이는 {{2}, {2, 1}, {2, 1, 3}, {2, 1, 3, 4}}  와 같이 표현할 수 있습니다.  
이때, 집합은 원소의 순서가 바뀌어도 상관없으므로 {{2}, {2, 1}, {2, 1, 3}, {2, 1, 3, 4}} {{2, 1, 3, 4}, {2}, {2, 1, 3}, {2, 1}} {{1, 2, 3}, {2, 1}, {1, 2, 4, 3}, {2}}는 모두 같은 튜플 (2, 1, 3, 4)를 나타냅니다.  
...
public class Main {
    ...
    static class Solution {
        ...
        public int[] solution(String s) {
            ArrayList<Integer> arrayList = new ArrayList<>();
            List<List<Integer>> output = parseStringToListOfIntegerLists(s);
            for (int i = 0; i < output.size(); i++) {
                for (List<Integer> integers : output) {
                    if (integers.size() == 1) {
                        arrayList.add(integers.get(0));
                        break;
                    }
                }
                for (List<Integer> integers : output) {
                    if (integers.contains(arrayList.get(arrayList.size() - 1))) {
                        integers.remove(arrayList.get(arrayList.size() - 1));
                    }
                }
            }
        ...
        public static List<List<Integer>> parseStringToListOfIntegerLists(String str) {
            // 최외각 중괄호 제거
            ...
            for (String part : parts) {
                ...
                for (String number : numbers) {
                    ...
                    subList.add(Integer.parseInt(number.trim()));
                }
                list.add(subList);
            }
...
#### 그라파나 설치
도커등 구동하면 3000번 포트가 기본  
아디비번 admin, admin이 기본
사이트 내부에서 프로메테우스와 연동  

#### 그라파나 대시보드 만들기
gui로 생성해서 뭐 그냥 대시보드에 사용할 프로메테우스 문법들 넣으면 그래프 보이는것  
gui이기 때문에 그냥 사용법 영상으로 대강 확인하는게 좋음  

#### 그라파나 공유 대시보드  
결국 사용하는게 비슷하기때문에 미리 만들어둔걸 가져와서 쓸 수 있음  
크라파나 사이트에서 `JVM (Micrometer)``Spring Boot 2.1 System Monitor`이거 두개 많이 사용  
